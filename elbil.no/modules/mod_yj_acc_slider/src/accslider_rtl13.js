...
var FancySlider = new Class({
    ...
    changeSlide: function (b) {
        this.slides[b]["fx"].start({
            ...
            right: this.slides[b]["right"] - (b == 0 ? 0 : this.options.openedWidth - this.options.closedWidth)
        });
        ...
        if (this.current < b) {
            for (var a = this.current; a < b; a++) {
                this.slides[a]["fx"].start({
                    ...
                    right: a * this.options.closedWidth
                })
            }
        } else {
            if (this.current > b) {
                ...
                for (var a = b + 1; a < this.slides.length; a++) {
                    this.slides[a]["fx"].start({
                        width: this.options.closedWidth,
                        right: (a - 1) * this.options.closedWidth + this.options.openedWidth
                    })
                }
            }
        }
