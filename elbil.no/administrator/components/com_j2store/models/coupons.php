 ...
 * @since 2.5
 */
class J2StoreModelCoupons extends J2StoreModel
{
	/**
	 *
	 * @var array
	...
	function _buildContentOrderBy()
	{
		...
		$orderby 	= ' ORDER BY '.$filter_order.' '.$filter_order_Dir.' , a.coupon_id';
	...
	function _buildContentWhere()
	{
		...
		if ($search) {
			$where[] = 'LOWER(a.coupon_name) LIKE '.$db->Quote( '%'.$db->escape( $search, true ).'%', false )
						.'OR LOWER(a.coupon_code) LIKE '.$db->Quote( '%'.$db->escape( $search, true ).'%', false )
						.'OR LOWER(a.value) LIKE '.$db->Quote( '%'.$db->escape( $search, true ).'%', false )
						;
		}
