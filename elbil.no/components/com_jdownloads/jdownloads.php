    ...
    if ($files){
        ...
        if ($jlistConfig['use.alphauserpoints']){
            if ($jlistConfig['use.alphauserpoints.with.price.field']){
                ...
                if ($profil->points >= $sum_points){
                    foreach($rows as $aup_data){
                        ...
                        if ($price = (int)$database->loadResult()){
                            $can_download = setAUPPointsDownloads($user->id, $aup_data->file_title, $aup_data->file_id, $price, $profil);
                        }
                    }
                }
                ...
                if ($profil->points >= $sum_points){
                    foreach($rows as $aup_data){
                        $can_download = setAUPPointsDownloads($user->id, $aup_data->file_title, $aup_data->file_id, 0, $profil);
                    }
                } else {
                    $can_download = false;
                }    
        ...
        if (!$may_download){
           ...
           $can_download = setAUPPointsDownload($user->id, $file_title, $fileid, $price, $profil);
           if ($jlistConfig['user.can.download.file.when.zero.points'] && $user->id){
               $can_download = true;
           }    
           if (!$can_download){
...
function setAUPPointsDownload($user_id, $file_title, $file_id, $price, $profile = array()){
    // added (or reduce) points to the alphauserpoints when is activated in the jD config
            ...
            if ($referreid){
                ...
                if ($jlistConfig['user.can.download.file.when.zero.points'] || $profile->points > 0 || $price == 0){
                    if ($price){
                        ...
                        // 19.8.2014: why is this deactivated???
...
function setAUPPointsDownloads($user_id, $file_title, $file_id, $price, $profile = array()){
    // added (or reduce) points to the alphauserpoints when is activated in the jD config
    // $user_id = user ID from the file download
    global $jlistConfig;
...
function escape_string($str) {
    ...
    * 
    */
    function fileIsPicture($filename)
    {
        jimport( 'joomla.filesystem.file' );
    ...
    */    
    ...
    function isBadImageFile($file)
    {
        if (file_exists($file))
        {
          ...
          //  Other routine but to restrictively?
