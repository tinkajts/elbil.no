...
function viewUpload($option, $view ){
	...
	if ($sended == 1 && $access) { 
		JSession::checkToken('request') or jexit( 'Invalid Token - Please load the page new!' ); 
		$no_valid = 0; 
		if ($name != '') {
			$name_pic =	'';
		} else {
		...
		if ($no_valid == 0) {
	       ...
	       if ($file_upload['tmp_name'] != '') { 
               ...
               // when we have an image mimetype, we must check the file
               $is_valid_image = true;
               $is_image = fileIsImage($file_upload['type']);
               if ($is_image){
                   // search bad code in file
                   $is_valid_image = imageFileIsValid($file_upload['tmp_name']);                    
               }

               if (!in_array($filetype, $file_types) || stristr($file_upload['name'], '.php.') || !$is_valid_image){
                   // error - user have tried to upload a not allowed file type
           		   // remove also all selected pic files
                   $file_upload['tmp_name'] = '';
                   $pic_upload['tmp_name'] = '';
                   $pic_upload2['tmp_name'] = '';
                   $pic_upload3['tmp_name'] = '';
                   $extern_file = '';
           ...
           if ($extern_file != '') { 
               ...
               if ($filetype){
                   $file_types = trim($allowed_file_types);
                   $file_types = explode(',', $file_types);
                   if (!in_array($filetype, $file_types) || stristr($extern_file, '.php.')){                    
                        $extern_file = '';
                        $msg = '<div>'.$upload_stop_pic.'<font color="red"> '
                           .JText::_('COM_JDOWNLOADS_FRONTEND_UPLOAD_ERROR_FILETYPE').
                           '</font><br />&nbsp;</div>';
            ...
            $pic_types = 'gif,jpg,jpeg,png';
            $pic_types = explode(',', $pic_types);
            
            if($pic_upload['tmp_name'] != ''){
               $pictype = strtolower(substr(strrchr($pic_upload['name'],"."),1)); 
               if (in_array($pictype, $pic_types) && !stristr($pic_upload['name'], '.php.')){
                  // replace special chars in filename
                  $pic_filename = JFile::makeSafe(basename($pic_upload['name']));
                  // search bad code in file
                  if (imageFileIsValid($pic_upload['tmp_name']) === true){
            ...
            if($pic_upload2['tmp_name'] != ''){
               ...
               if (in_array($pictype, $pic_types) && !stristr($pic_upload2['name'], '.php.')){
                  // replace special chars in filename
                  $pic_filename = JFile::makeSafe(basename($pic_upload2['name']));
                  // search bad code in file
                  if (imageFileIsValid($pic_upload2['tmp_name']) === true){
                      $only_name = substr($pic_filename, 0, strrpos($pic_filename, '.'));
                      $file_extension = strrchr($pic_filename,".");
                      $num = 0;
                      while (is_file(JPATH_SITE.$upload_dir.$pic_filename)){
            ...
            if($pic_upload3['tmp_name'] != ''){
               ...
               if (in_array($pictype, $pic_types) && !stristr($pic_upload3['name'], '.php.')){
                  // replace special chars in filename
                  $pic_filename = JFile::makeSafe(basename($pic_upload3['name']));
                  // search bad code in file
                  if (imageFileIsValid($pic_upload3['tmp_name']) === true){
                      $only_name = substr($pic_filename, 0, strrpos($pic_filename, '.'));
                      $file_extension = strrchr($pic_filename,".");
                      $num = 0;
                      while (is_file(JPATH_SITE.$upload_dir.$pic_filename)){
			...
			if($file_upload['tmp_name'] != ''){
                // replace special chars in filename
                $filename_new = checkFileName(basename($file_upload['name']));
